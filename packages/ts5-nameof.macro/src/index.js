...
import { transformNode } from "@ts5-nameof/transforms-babel";
...
function nameofMacro({ references, state, babel }) {
  ...
  reverseDefault.forEach(path => {
    ...
    function getPath() {
      ...
      throw new MacroError("[ts5-nameof]: Could not find a call expression at path: " + grandParentPath.getSource());
    }
  });
}
